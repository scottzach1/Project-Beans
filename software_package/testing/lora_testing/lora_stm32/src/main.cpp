...
// Store the transmission error state. This is global such that it can be
// utilised when the ISR fires upon transmission completion.
int txState = 0;
...
// Transmit a String
void transmitPacket(String message) { txState = radio.startTransmit(message); }
...
void setup() {
    ...
    // set the function that will be called when packet transmission is
    // finished.
...
void loop() {
    ...
    // This is the minimum delay or there will be a CRC error on the receiving
    // end
    delay(50);
}
